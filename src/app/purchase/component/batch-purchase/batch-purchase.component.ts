...
export class BatchPurchaseComponent implements OnInit {
  ...
  private setBatchValues() {
    this.defaultProductFrom.controls.prefixCode.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef), filter(Boolean))
      .subscribe(prefixCode => {
        this.batchPurchaseForm.controls.forEach((control, index) => {
          control.controls.code.setValue(this.generateCode(prefixCode, index));
        });
      });
    this.defaultProductFrom.controls.productName.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef), filter(Boolean))
    ...
    this.defaultProductFrom.controls.productDescription.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef), filter(Boolean))
    ...
    this.defaultProductFrom.controls.defaultPurchasePrice.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef), filter(Boolean))
      .subscribe(defaultPurchasePrice => {
        this.batchPurchaseForm.controls.forEach(control => {
          control.controls.purchaseUnitPrice.setValue(defaultPurchasePrice);
        });
      });
    this.defaultProductFrom.controls.defaultSellPrice.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef), filter(Boolean))
      .subscribe(defaultSellPrice => {
        this.batchPurchaseForm.controls.forEach(control => {
  ...
  private subscribeToPurchaseInvoiceFormAndUpdatePaidPrice() {
    combineLatest([
      this.purchaseInvoiceForm.controls.totalPrice.valueChanges,
      this.purchaseInvoiceForm.controls.discount.valueChanges.pipe(startWith(0))
    ])
      .pipe(
      ...
      .subscribe(paidPrice => {
        this.purchaseInvoiceForm.controls.paidPrice.setValue(paidPrice, {
          emitEvent: false
        });
      });
  }
  ...
  private subscribeToBatchPurchaseFormAndUpdateTotalPrice() {
    this.batchPurchaseForm.valueChanges
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        map(forms => {
